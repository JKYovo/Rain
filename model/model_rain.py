...
class Attention(nn.Module):
    ...
    """
    def __init__(self, args: RainConfig):
        super().__init__()
        ...
        # QKV 投影层, q:512->512, k:512->128, v:512->128, o:512->512    
    ...
    def forward(self,
        ...
        # hidden_size = num_kv_heads * head_dim, 进行最后一个维度的分头处理
        ...
        # KV cache 实现（用于推理加速）， 训练用不到
        if past_key_value is not None:
            xk = torch.cat([past_key_value[0], xk], dim=1) # 将 past_key_value 的 key 和当前的 key 拼接起来， 方便下一个token的推理
            xv = torch.cat([past_key_value[1], xv], dim=1) # 将 past_key_value 的 value 和当前的 value 拼接起来
        past_kv = (xk, xv) if use_cache else None

        # 转置并重复 KV（用于 GQA），把 Key 和 Value 的头数重复到和 Query 头数一样多
        # 以便后续做标准的 scaled dot-product attention 计算
        # (batch, seq_len, num_heads, head_dim) -> (batch, num_heads, seq_len, head_dim)， 按 num_heads 维度计算每个头注意力
        xq, xk, xv = (
            xq.transpose(1, 2),
        ...
        # 恢复形状并输出投影，batch, num_heads, seq_len, head_dim -> batch, seq_len, hidden_size
...
class RainBlock(nn.Module):
    ...
    def forward(self, hidden_states, position_embeddings, past_key_value=None, use_cache=False, attention_mask=None):
        ...
        residual = hidden_states # 保存原始输入用于残差
...
class RainModel(nn.Module):
    """
    Rain 模型主体（Decoder-only Transformer）
    """
    def __init__(self, config: RainConfig):
        super().__init__()
        self.config = config
        ...
        # Transformer Blocks
        self.layers = nn.ModuleList([RainBlock(l, config) for l in range(self.num_hidden_layers)])

...
def calculate_rewards(raw_prompts, responses, args):
    ...
    for i in range(num_resp):
        ...
        if format_ok[i] and i in judge_results:
            ...
            rewards[i] = sum(scores.values()) / 3.0 # 三个维度的打分
            judge_scores = scores
...
vocab = {0:"我", 1:"是", 2:"Grok", 3:"。"}
completion 部分是：我是Grok。 → token ids = [1, 2, 3]
模型对这三个位置预测的 logits（简化）：

位置1（预测“是”）： logits = [0.1, 5.0, 0.2, 0.1] → log_softmax ≈ [很小, -0.01, 很小, 很小]
位置2（预测“Grok”）： logits = [0.1, 0.1, 6.0, 0.1] → log_softmax ≈ [很小, 很小, -0.01, 很小]
位置3（预测“。”）： logits = [0.1, 0.1, 0.1, 4.0] → log_softmax ≈ [很小, 很小, 很小, -0.05]
gather 后得到：
logprobs ≈ [-0.01, -0.01, -0.05]
→ 这三个值就是模型对“我是Grok。”这句话每个词的“自信度”（log 概率，越接近 0 越自信）
"""
...
# logprobs 只应该算到第一个 EOS 之前（后面可能是模型胡编或 padding）
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Rain GRPO Training")
    ...
    # 路径
    parser.add_argument("--data_path", type=str, default="/root/autodl-tmp/Rain/GRPO_train.jsonl")
    parser.add_argument("--tokenizer_path", type=str, default="/root/autodl-tmp/Rain/tokenizer_15k")
    parser.add_argument("--sft_model_path", type=str, 
                       default="/root/autodl-tmp/Rain/dir/thinking_distill_768.pth")

...
from model.config import RainConfig
...
def train_epoch(epoch, loader, iters, start_step=0, swanlab=None, total_steps=None, warmup_steps=None, full_save_dir=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        # 梯度累积，计算梯度，比如一次算128的batch，改为4次算32的batch，这样梯度不会太大，等价
        if (step + 1) % args.accumulation_steps == 0:
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip) # 裁剪梯度，防止梯度剧烈震荡

            scaler.step(optimizer)
            scaler.update()

            optimizer.zero_grad(set_to_none=True) # 梯度清零，防止累积
        ...
        if (global_step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            model.eval() # 进入评估/推理模式，把所有训练专用的行为关掉，保证输出稳定、可复现
            ...
            # 保存一个完整的“可恢复训练”检查点（resume checkpoint），方便中断后继续训练
            torch.save({
                # 模型权重（state_dict）
                # 这里保存的是已经处理好的模型参数字典（通常是 .cpu() 且可能转成 half/bf16 的版本）
                'model': state_dict,
                
                # 优化器状态
                # 包含动量（momentum）、学习率调度器的内部状态、Adam 的 exp_avg / exp_avg_sq 等
                # 如果不保存，下次加载后优化器会从头开始，相当于损失了前面几千步的动量信息，收敛会变慢
                'optimizer': optimizer.state_dict(),
                
                # 混合精度训练的梯度缩放器（GradScaler）状态
                # 保存了当前 scale 值、growth_interval、上次增长/缩小的历史等
                # 如果使用 torch.cuda.amp / autocast 训练，不保存 scaler 会导致下次继续训练时 scale 从 65536.0 重新开始
                # 可能造成最初几步 loss 爆炸或不稳定
                'scaler': scaler.state_dict(),
                
                # 当前训练的 epoch 编号（从 0 或 1 开始）
                # 用于记录训练到了第几轮，方便日志和断点续训后继续正确的 epoch 计数
                'epoch': epoch,
                
                # 当前 epoch 内的 step（批次）编号
                # 有些项目用它来计算总的训练步数或做更细粒度的日志
                'step': step,
                
                # 全局训练步数（global step）
                # 通常是累加所有 epoch 的总 batch 数，常用于学习率调度（cosine decay、warmup 等）
                # 保存它可以让 lr_scheduler 在加载后从正确的位置继续
                'global_step': global_step,
                
                # 如果使用了 SwanLab（一种轻量实验跟踪工具，类似 wandb 的国内替代）
                # 保存实验的唯一 ID，方便断点续训后继续关联到同一个实验页面
                # getattr + 判断是为了防止 swanlab 模块没导入或没初始化时出错
...
if __name__ == "__main__":
    ...
    # ========== 2. 配置目录、模型参数、检查ckp ==========
    lm_config = RainConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers)
